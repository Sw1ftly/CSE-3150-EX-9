...
public:
    ...
    double distance(const Point3d& s, const Point3d& t) const {
        return std::sqrt((s.get_x() - t.get_x()) * (s.get_x() - t.get_x()) +
                         (s.get_y() - t.get_y()) * (s.get_y() - t.get_y()) +
                         (s.get_z() - t.get_z()) * (s.get_z() - t.get_z()));
    }
